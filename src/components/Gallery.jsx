    ...
    <section id="galleri" className="py-16 sm:py-20">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex items-end justify-between">
          ...
          <p className="text-white/70 text-sm max-w-md">Få et indblik i stemningen, vi skaber.</p>
        </div>
        <div className="mt-8 grid grid-cols-2 md:grid-cols-3 gap-3">
          {images.map((src, idx) => (
            <button key={src} onClick={() => setActive(src)} className="group relative overflow-hidden rounded-xl">
              <img src={src} alt="Event" className="aspect-[4/3] w-full object-cover group-hover:scale-105 transition" />
